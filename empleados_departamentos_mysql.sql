...
create procedure insertar_empleado(
    in p_ndiemp varchar(12),
    in p_nomemp varchar(30),
    in p_sexemp char(1),
    in p_fecnac date,
    in p_fecincorporacion date,
    in p_salemp float,
    in p_comisione float,
    in p_cargoe varchar(15),
    in p_jefeid varchar(12),
    in p_coddepto varchar(4)
)
begin
...
create procedure buscar_empleado(
    in p_busqueda varchar(30)
)
begin
    select * from empleados 
    where ndiemp like concat('%', p_busqueda, '%') 
       or nomemp like concat('%', p_busqueda, '%');
end //
...
begin
    update empleados set
        ...
        sexemp = p_sexemp,
        fecnac = p_fecnac,
        fecincorporacion = p_fecincorporacion,
        salemp = p_salemp,
        comisione = p_comisione,
        cargoe = p_cargoe,
        jefeid = p_jefeid,
        coddepto = p_coddepto
    where ndiemp = p_ndiemp;
end //
...
begin
    delete from empleados where ndiemp = p_ndiemp;
end //
